...
## 소스코드의 타입
ECMAScript 사양은 소스코드를 4가지 타입으로 구분. 이들은 실행 컨텍스트를 생성
...
var x는 먼저 실행하여 실행 컨텍스트가 관리하는 스코프에 등록되고 undefined로 초기화  
...
- 선언적 환경 레코드 생성 : const는 선언 단계와 초기화 단계가 분리되어 진행. 따라서 초기화 단계, 즉 런타임에 실행 흐름이 변수 선언문에 도달하기 전까지 일시적 사각지대(Temporal Dead Zone)에 빠지게 된다.
- this 바인딩 : this바인딩은 객체 환경 레코드와 선언적 환경 레코드에는 없고, 전역 환경 레코드와 함수 환경 레코드에만 존재한다.
- 외부 렉시컬 환경에 대한 참조 결정 : 상위 스코프를 가리키며 단방향 링크드 리스트인 스코프 체인을 구현 (전역 코드시는 null)
### 전역 코드 실행
- 식별자는 스코프가 다르면 같은 이름을 가질 수 있으므로, 식별자 결정이 필요  
- 실행 중인 실행 컨테슥트에서 식별자를 검색하기 시작.  
- 전역까지 해당 식별자가 없다면 참조 에러 발생

## 실행 컨텍스트와 블록 레벨 스코프
var : 함수의 코드 블록만 지역 스코프 (함수 레벨 스코프)
let, const : 모든 코드 블록을 지역 스코프로 인정 (블록 레벨 스코프)
